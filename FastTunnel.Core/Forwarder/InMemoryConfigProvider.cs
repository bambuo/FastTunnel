...
/// </summary>
public class FastTunnelInMemoryConfigProvider(IReadOnlyList<RouteConfig> routes, IReadOnlyList<ClusterConfig> clusters)
    : IProxyConfigProvider
{
    ...
    public void AddWeb(string hostName)
    {
        lock (_locker)
        {
            ...
            newRoutes.Add(new RouteConfig { ClusterId = hostName, RouteId = hostName, Match = new RouteMatch { Hosts = new[] { hostName } } });

            var newClusters = oldConfig.Clusters.ToList();
            newClusters.Add(new ClusterConfig { ClusterId = hostName, Destinations = new Dictionary<string, DestinationConfig>(StringComparer.OrdinalIgnoreCase) { { "default", new DestinationConfig { Address = $"http://{hostName}" } } } });
