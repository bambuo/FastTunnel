...
export interface TunnelOpResult<T> {
  data: T
  message?: string
}
...
export async function createWebTunnel(data: WebTunnelRequest): Promise<TunnelOpResult<WebTunnel>> {
  const res = await request.post<ApiResponse<WebTunnel>>('/tunnels/webs', data)
  if (!res.data.success) throw new Error(res.data.message)
  return { data: res.data.data, message: res.data.message }
}

export async function updateWebTunnel(id: number, data: WebTunnelRequest): Promise<TunnelOpResult<WebTunnel>> {
  const res = await request.put<ApiResponse<WebTunnel>>(`/tunnels/webs/${id}`, data)
  if (!res.data.success) throw new Error(res.data.message)
  return { data: res.data.data, message: res.data.message }
}

export async function deleteWebTunnel(id: number): Promise<TunnelOpResult<null>> {
  const res = await request.delete<ApiResponse<null>>(`/tunnels/webs/${id}`)
  if (!res.data.success) throw new Error(res.data.message)
  return { data: res.data.data, message: res.data.message }
}

export async function toggleWebTunnel(id: number, isEnabled: boolean): Promise<TunnelOpResult<{ id: number; isEnabled: boolean }>> {
...
export async function createForwardTunnel(data: ForwardTunnelRequest): Promise<TunnelOpResult<ForwardTunnel>> {
  const res = await request.post<ApiResponse<ForwardTunnel>>('/tunnels/forwards', data)
  if (!res.data.success) throw new Error(res.data.message)
  return { data: res.data.data, message: res.data.message }
}

export async function updateForwardTunnel(id: number, data: ForwardTunnelRequest): Promise<TunnelOpResult<ForwardTunnel>> {
  const res = await request.put<ApiResponse<ForwardTunnel>>(`/tunnels/forwards/${id}`, data)
  if (!res.data.success) throw new Error(res.data.message)
  return { data: res.data.data, message: res.data.message }
}

export async function deleteForwardTunnel(id: number): Promise<TunnelOpResult<null>> {
  const res = await request.delete<ApiResponse<null>>(`/tunnels/forwards/${id}`)
  if (!res.data.success) throw new Error(res.data.message)
  return { data: res.data.data, message: res.data.message }
}

export async function toggleForwardTunnel(id: number, isEnabled: boolean): Promise<TunnelOpResult<{ id: number; isEnabled: boolean }>> {
